class Permutation:
    ...
    def permute(p):
        return Permutation._alphabet.toInt(permuteHelper(_alphabet.toChar(wrap(p))))

    # Return the result of applying the inverse of this permutation to C modulo the alphabet size.
    def invert(c):
        return Permutation._alphabet.toInt(invertHelper(_alphabet.toChar(wrap(c))))
    ...
    def permuteHelper(p):
        ...
        if not (self._alphabet.contain(p)):
